...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="bksong1122/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/bksong1122/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage ('JMeter LoadTest') {
            steps { 
                sh '~/lab/sw/jmeter/bin/jmeter.sh -j jmeter.save.saveservice.output_format=xml -n -t src/main/jmx/guest_loadtest.jmx -l loadtest_result.jtl' 
